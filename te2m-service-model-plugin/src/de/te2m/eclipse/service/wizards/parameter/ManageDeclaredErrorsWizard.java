/*
* ManageDeclaredErrorsWizard.java
*   
 ...
 * @since 1.0
 */
public class ManageDeclaredErrorsWizard extends Wizard implements INewWizard {
	 ...
	 * @param spn the spn
	 */
	public ManageDeclaredErrorsWizard(ReturnsErrors obj, SimpleParameterNode spn) {
		super();
	...
	private void doFinish(String name, String desc, String type, String pkg,
		...
		ParameterInfo error = null;
		ParameterInfo tmpError = null;
		
		if(null!=tpn)
		{
			error=tpn.getParameterInfo();
			tmpError=tpn.getParameterInfo();
		}		
		if(null==error)
		{
			error=new ParameterInfo();
		}
		...
		ci.setPkg(pkg);
		ci.setName(type);
		error.setParamClass(ProjectModelProvider.getInstance().getProjectNode()
				.validateClassInfo(ci));
		((ReturnsErrors) on).addDeclaredError(error);
		if(null!=tmpError)
		{
			((ReturnsErrors) on).getDeclaredErrors().remove(tmpError);
		}
		monitor.worked(1);
	}
