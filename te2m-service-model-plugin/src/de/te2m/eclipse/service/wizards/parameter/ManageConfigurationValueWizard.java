/*
 * ManageConfigurationValueWizard.java
 *   
...
import de.te2m.eclipse.service.model.tree.service.Configurable;
...
/**
 * The Class ManageConfigurationValueWizard.
 *
 * @author ffischer
 * @version 1.0
 * @since 1.0
 */
public class ManageConfigurationValueWizard extends Wizard implements
		INewWizard {
	 ...
	 *            the spn
	 */
	public ManageConfigurationValueWizard(Configurable obj,
			SimpleParameterNode spn) {
		super();
	...
	private void doFinish(String name, String desc, String type, String pkg,
		...
		ParameterInfo cfg = null;
		ParameterInfo tmpCfg = null;

		if (null != tpn) {
			cfg = tpn.getParameterInfo();
			tmpCfg = tpn.getParameterInfo();
		}
		if (null == cfg) {
			cfg = new ParameterInfo();
		}
		...
		ci.setPkg(pkg);
		ci.setName(type);
		cfg.setParamClass(ProjectModelProvider.getInstance().getProjectNode()
				.validateClassInfo(ci));
		if (null != on) {

			on.addConfigValue(cfg);
			if (null != tmpCfg) {
				on.getConfiguration().remove(tmpCfg);
			}
		}
		monitor.worked(1);
	}
