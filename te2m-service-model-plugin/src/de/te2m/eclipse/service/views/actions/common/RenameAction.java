 ...
 * @since 1.0
 */
public class RenameAction extends AbstractTreeAction {
	 ...
	 * @param v the v
	 */
	public RenameAction(TreeViewer v) {
		super();
		myViewer = v;
		setText("Rename");
		setToolTipText("Rename this element");
	}
	...
	public void run() {
		...
		if (obj instanceof TreeNode) {
			...
			InputDialog inpD = new InputDialog(
					myViewer.getControl().getShell(), "Rename",
					"Rename Element", tn.getName(), null);

			inpD.open();

			if (inpD.getReturnCode() == Window.OK) {
				tn.setName(inpD.getValue());
				//tn.refresh();
				myViewer.refresh();
			}
		}
